/*
SCP Kubernetes Service

SCP Kubernetes Service

API version: 0.0.1.dev2582
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package ske

import (
	"bytes"
	"encoding/json"
	"fmt"
)

// checks if the ConfigMapListResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ConfigMapListResponse{}

// ConfigMapListResponse struct for ConfigMapListResponse
type ConfigMapListResponse struct {
	ConfigMaps []ConfigMapSummary `json:"config_maps"`
	Count      NullableInt32      `json:"count,omitempty"`
	Links      []interface{}      `json:"links,omitempty"`
}

type _ConfigMapListResponse ConfigMapListResponse

// NewConfigMapListResponse instantiates a new ConfigMapListResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewConfigMapListResponse(configMaps []ConfigMapSummary) *ConfigMapListResponse {
	this := ConfigMapListResponse{}
	this.ConfigMaps = configMaps
	return &this
}

// NewConfigMapListResponseWithDefaults instantiates a new ConfigMapListResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewConfigMapListResponseWithDefaults() *ConfigMapListResponse {
	this := ConfigMapListResponse{}
	return &this
}

// GetConfigMaps returns the ConfigMaps field value
func (o *ConfigMapListResponse) GetConfigMaps() []ConfigMapSummary {
	if o == nil {
		var ret []ConfigMapSummary
		return ret
	}

	return o.ConfigMaps
}

// GetConfigMapsOk returns a tuple with the ConfigMaps field value
// and a boolean to check if the value has been set.
func (o *ConfigMapListResponse) GetConfigMapsOk() ([]ConfigMapSummary, bool) {
	if o == nil {
		return nil, false
	}
	return o.ConfigMaps, true
}

// SetConfigMaps sets field value
func (o *ConfigMapListResponse) SetConfigMaps(v []ConfigMapSummary) {
	o.ConfigMaps = v
}

// GetCount returns the Count field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ConfigMapListResponse) GetCount() int32 {
	if o == nil || IsNil(o.Count.Get()) {
		var ret int32
		return ret
	}
	return *o.Count.Get()
}

// GetCountOk returns a tuple with the Count field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ConfigMapListResponse) GetCountOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return o.Count.Get(), o.Count.IsSet()
}

// HasCount returns a boolean if a field has been set.
func (o *ConfigMapListResponse) HasCount() bool {
	if o != nil && o.Count.IsSet() {
		return true
	}

	return false
}

// SetCount gets a reference to the given NullableInt32 and assigns it to the Count field.
func (o *ConfigMapListResponse) SetCount(v int32) {
	o.Count.Set(&v)
}

// SetCountNil sets the value for Count to be an explicit nil
func (o *ConfigMapListResponse) SetCountNil() {
	o.Count.Set(nil)
}

// UnsetCount ensures that no value is present for Count, not even an explicit nil
func (o *ConfigMapListResponse) UnsetCount() {
	o.Count.Unset()
}

// GetLinks returns the Links field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *ConfigMapListResponse) GetLinks() []interface{} {
	if o == nil {
		var ret []interface{}
		return ret
	}
	return o.Links
}

// GetLinksOk returns a tuple with the Links field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *ConfigMapListResponse) GetLinksOk() ([]interface{}, bool) {
	if o == nil || IsNil(o.Links) {
		return nil, false
	}
	return o.Links, true
}

// HasLinks returns a boolean if a field has been set.
func (o *ConfigMapListResponse) HasLinks() bool {
	if o != nil && !IsNil(o.Links) {
		return true
	}

	return false
}

// SetLinks gets a reference to the given []interface{} and assigns it to the Links field.
func (o *ConfigMapListResponse) SetLinks(v []interface{}) {
	o.Links = v
}

func (o ConfigMapListResponse) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ConfigMapListResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["config_maps"] = o.ConfigMaps
	if o.Count.IsSet() {
		toSerialize["count"] = o.Count.Get()
	}
	if o.Links != nil {
		toSerialize["links"] = o.Links
	}
	return toSerialize, nil
}

func (o *ConfigMapListResponse) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"config_maps",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varConfigMapListResponse := _ConfigMapListResponse{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varConfigMapListResponse)

	if err != nil {
		return err
	}

	*o = ConfigMapListResponse(varConfigMapListResponse)

	return err
}

type NullableConfigMapListResponse struct {
	value *ConfigMapListResponse
	isSet bool
}

func (v NullableConfigMapListResponse) Get() *ConfigMapListResponse {
	return v.value
}

func (v *NullableConfigMapListResponse) Set(val *ConfigMapListResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableConfigMapListResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableConfigMapListResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableConfigMapListResponse(val *ConfigMapListResponse) *NullableConfigMapListResponse {
	return &NullableConfigMapListResponse{value: val, isSet: true}
}

func (v NullableConfigMapListResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableConfigMapListResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
